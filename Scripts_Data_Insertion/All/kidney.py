	...
	if patid not in pat_id:
		...
		if currentcontact not in contact:
			...
			if(address not in addresslist):
				...
				sql = "INSERT INTO  Patient (Pincode, Last_check_up_date , PAT_id , Name , Contact , Address , Doc_id , Organ , Age , Blood_Group) VALUES (%s, %s,%s,%s,%s, %s,%s,%s,%s, %s)"
				val=  (address[-1], date , patid , names.get_full_name() , currentcontact , address[0]+" "+address[1]+" "+address[2] , Doc_id , organ , Age , Blood_Group)
