...
organizationlist=pickle.load(ld)
...
for i in organizationlist:
	...
	if i[1]=="Kidney":
		...
		oPancreas.append(i)
organisationidlis=[oKidney,oLiver,oCornea,oHeart,oLung,oPancreas]
# print(organisationidlis)
...
for i in range(0,100):
	...
	if patid not in pat_id:
		...
		if currentcontact not in contact:
			...
			date=random_date("2019-12-31", "2020-4-20", random.random())
			# print(date)
			...
			if(address not in addresslist):
				...
				# val=rand.randint(0,maxlistindex-1)
				# # print("Organ= ",organ)
				# Doc_id=doctororgan[organindex][val][0]
				maxlistindex=len(organisationidlis[organindex])	
				val=rand.randint(0,maxlistindex-1)
				ORG_id=organisationidlis[organindex][val][0]
				# print(val,maxlistindex)
				# print(ORG_id,organ)
				# print(Doc_id)
				Blood_Group=Bloodgroup[rand.randint(0,7)]
				Age=rand.randint(18,50)
				name=names.get_full_name()
				# print(address[-1], date , patid , name , currentcontact , address[0]+" "+address[1]+" "+address[2] , Doc_id , organ , Age , Blood_Group)

				sql = "INSERT INTO  Organ_Donor (Pincode, Last_check_up_date , DON_id , Name , Contact , Address , ORG_id  , Age , Blood_Group,Status) VALUES (%s, %s,%s,%s,%s, %s,%s,%s,%s, %s)"
				val=  (address[-1], date , patid , name , currentcontact , address[0]+" "+address[1]+" "+address[2] , ORG_id , Age , Blood_Group,"Alive")
				mycursor.execute(sql, val)
				sql = "INSERT INTO  Donors_Organ (DON_id, Organ ) VALUES (%s, %s)"
				val=  (patid, organ)
				mycursor.execute(sql, val)
				temp=[patid,organ]
				donoridlist.append(temp)

				mydb.commit()

				print(mycursor.rowcount, "record inserted.")

with open('organdonor.list', 'wb') as f:
		pickle.dump(donoridlist, f)
